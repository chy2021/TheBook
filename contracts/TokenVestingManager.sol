    ...
    contract TokenVestingManager is Ownable, ReentrancyGuard {
        ...
        /**
         * @notice Batch create vestings. All arrays must have the same length.
         */
        function createVestingBatch(
            address[] calldata beneficiaries,
            uint256[] calldata amounts,
            uint64[] calldata cliffSeconds,
            uint64[] calldata durationSeconds,
            bool[] calldata monthly,
            uint32[] calldata monthsArr,
            uint64[] calldata monthSecondsArr
        ) external onlyOwner nonReentrant {
            uint256 n = beneficiaries.length;
            require(n > 0, "empty input");
            require(
                amounts.length == n && cliffSeconds.length == n && durationSeconds.length == n && monthly.length == n && monthsArr.length == n && monthSecondsArr.length == n,
                "array length mismatch"
            );

            for (uint256 i = 0; i < n; i++) {
                _createVesting(beneficiaries[i], amounts[i], cliffSeconds[i], durationSeconds[i], monthly[i], monthsArr[i], monthSecondsArr[i]);
            }
        }
