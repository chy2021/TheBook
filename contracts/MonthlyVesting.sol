 ...
 * @notice 将总额分为若干个月度等额份额（最后一期包含余数），并允许任何人调用 release()
 * 设计假设：start 表示第一个月份的释放时点（即 Cliff 结束后的时刻），在 start 时可领取第一期。
 */
contract MonthlyVesting is ReentrancyGuard {
    ...
    uint64 public immutable start; // first month unlock time (seconds)
    uint64 public immutable monthSeconds; // seconds per month (e.g., 30 days)
    uint32 public immutable months; // total months
    uint256 public immutable totalAmount; // total token allocated
    uint256 public immutable monthlyAmount; // integer division
    uint256 public immutable remainder; // totalAmount - monthlyAmount * months

    uint32 public releasedMonths; // how many months already released
